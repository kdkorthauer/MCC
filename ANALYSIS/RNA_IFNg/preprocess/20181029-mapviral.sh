...
for i in /rafalab/keegan/MCC/DATA/RNA_IFNg/*2.bam; do
    [ -f "$i" ] || break
    samp=$(basename $i .bam)
    pref=$(dirname $i)
...
for i in /rafalab/keegan/MCC/DATA/RNA_IFNg/*3.bam; do
    [ -f "$i" ] || break
    samp=$(basename $i .bam)
    pref=$(dirname $i)
    if [[ ! -e $pref/$samp\_unmapped.bam ]]; then
...
for i in /rafalab/keegan/MCC/DATA/RNA_IFNg/*unmapped.bam; do
    [ -f "$i" ] || break
    samp=$(basename $i .bam)
    pref=$(dirname $i)
...
for i in /rafalab/keegan/MCC/DATA/RNA_IFNg/*virusmap.bam; do
    samp=$(basename $i)
    pref=$(dirname $i)
