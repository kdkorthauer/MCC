...
We'll also read in the RNA-seq count data from control cell lines (fibroblasts, keratinocytes, MKL1, and Waga) that were downloaded, aligned with STAR, and quantified with Subread.

```{r}
exp_control <- data.frame(read_tsv("../../PREPROCESS/RNA/control_lines/counts/mRNA-Seq.counts.txt"))
rownames(exp_control) <- exp_control[,1]
exp_control <- exp_control[, -c(1:6)]

colDat_control <- data.frame(sampleName = colnames(exp_control),
                             dfci.id = NA,
                             type = gsub("_.*", "", colnames(exp_control)),
                             mcc.id = NA,
                             virus = NA,
...
ifn.map <- rbind(c("275", NA),
                 ...
                 c("367", 8),
                 c("2314", 3))
ifn.map <- data.frame(ifn.map)
colnames(ifn.map) <- c("id1", "ifn")
ifn.map$ifn <- as.numeric(ifn.map$ifn)
x <- match(colDat$mcc.id, ifn.map$id1)

colDat$ifn <- ifelse(ifn.map$ifn[x] <= 4, "Responsive", "Non_responsive")
```
...
# including control lines
dat_control <- DESeqDataSetFromMatrix(countData = cbind(exp, exp_control),
                              colData = rbind(colDat[,-6], colDat_control),
                              rowData = rowDat,
                              design = ~ type)
```
...
```{r}
dat <- estimateSizeFactors(dat)
dat_control <- estimateSizeFactors(dat_control)
...
# with control
map.cont <- select(org.Hs.eg.db,
              keys = unlist(lapply(strsplit(rownames(dat_control), "\\."), 
                                       function(x) x[1])),
              columns=c("SYMBOL","GENENAME", "ENSEMBL"),
              keytype="ENSEMBL")
x <- match(unlist(lapply(strsplit(rownames(dat_control), "\\."), function(x) x[1])),
           map.cont$ENSEMBL)
rowData(dat_control)$symbol <- map.cont$SYMBOL[x]
...
rowData(dat_control)$housekeeping <- rowData(dat_control)$symbol %in% hkgenes$symbol
dat_control_hk <- RUVg(counts(dat_control), rowData(dat_control)$housekeeping, k=1)
assays(dat_control)$hk_norm <- dat_control_hk$normalizedCounts
```
...
vsd = vst(dat)
vsd_control = vst(dat_control)

# print table of vst values:
write.table(assay(vsd), quote=FALSE, file=file.path("results", "vst_expression_tumor.txt"), sep="\t", col.names=NA)

# print table of vst values:
write.table(assay(vsd_control), quote=FALSE, file=file.path("results", "vst_expression_tumor_withControlLines.txt"), sep="\t", col.names=NA)

# print housekeeping normed counts
write.table(assays(dat_control)$hk_norm, quote=FALSE, file=file.path("results", "housekeepingNormed_expression_tumor_withControlLines.txt"), sep="\t", col.names=NA)
...
```{r pairscatter, fig.width = 11, fig.height = 10, fig.path='plots/', dev = c('png'), eval=FALSE}
...
mat <- data.frame(log2(counts(dat, normalize = TRUE)+1))
...
Pairwise spearman correlations for vst expression

```{r}
dist_sp <- usedist::dist_make(t(assay(vsd[,-which(grepl("DFCI.5370.T", colnames(vsd)))])), 
  distance_fcn = cor, method = "spearman")
dist_sp <- as.matrix(dist_sp)
diag(dist_sp) <- NA

pairs_ix <- other_ix <- cell_ix <- tumor_ix <- matrix(0, nrow=nrow(dist_sp), ncol=ncol(dist_sp))
for (i in seq_along(rownames(dist_sp))){
  for (j in seq_along(colnames(dist_sp))){
    if(i < j){
      if( gsub("\\.C|\\.T|\\.CL", "", rownames(dist_sp))[i] ==
          gsub("\\.C|\\.T|\\.CL", "", colnames(dist_sp))[j]  ){
        pairs_ix[i,j] <- 1
      }else if( substr(gsub("L", "", rownames(dist_sp))[i], 11, 11) ==
                substr(gsub("L", "", rownames(dist_sp))[j], 11, 11) && 
                substr(gsub("L", "", rownames(dist_sp))[i], 11, 11) == "C" ){
        cell_ix[i,j] <- 1
      }else if( substr(gsub("L", "", rownames(dist_sp))[i], 11, 11) ==
                substr(gsub("L", "", rownames(dist_sp))[j], 11, 11) &&
                substr(gsub("L", "", rownames(dist_sp))[i], 11, 11) == "T"){
        tumor_ix[i,j] <- 1
      }else{
        other_ix[i,j] <- 1
      }
...
cordf <- data.frame(
  type = factor(c(rep("C+T pair", sum(pairs_ix)),
                  rep("C+C pair", sum(cell_ix)),
                  rep("T+T pair", sum(tumor_ix)),
                  rep("Other", sum(other_ix)))),
  spearman = c(dist_sp[which(pairs_ix==1, arr.ind=TRUE)],
               dist_sp[which(cell_ix==1, arr.ind=TRUE)],
               dist_sp[which(tumor_ix==1, arr.ind=TRUE)], 
               dist_sp[which(other_ix==1, arr.ind=TRUE)])
  )

cordf$type <- factor(cordf$type, levels(cordf$type)[c(2, 1, 4, 3)])


ggplot(cordf, aes(x=type, y=spearman)) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(alpha=0.2) + 
  xlab("Pair type") +
  ylab("Spearman rank correlation")
ggsave(file="plots/pairwise_spearman.pdf", width=5, height=4)

t.test(cordf$spearman[cordf$type=="C+T pair"], cordf$spearman[cordf$type=="Other"])
t.test(cordf$spearman[cordf$type=="C+T pair"], cordf$spearman[cordf$type=="C+C pair"])
t.test(cordf$spearman[cordf$type=="C+T pair"], cordf$spearman[cordf$type=="T+T pair"])
...
Including the control lines

```{r heatmap-dist-l2n-withcontrol-hk, fig.width=8.5, fig.height=7, fig.path='plots/', dev=c('png')}
cols2_control <-  colorRampPalette( rev(brewer.pal(length(unique(colData(dat_control)$mcc.id)), 
                                           "Set3")) )(length(unique(colData(dat_control)$mcc.id)))
names(cols2_control) <- unique(colData(dat)$mcc.id)
cols2_control <- cols2_control[-which(is.na(names(cols2_control)))]

vircol <- c("#543005", "#DFC27D", "white")
names(vircol) <- unique(colData(dat)$virus)
vircol <- vircol[-which(is.na(names(vircol)))]

ha_column_control = HeatmapAnnotation(df = data.frame(
                      Type = colData(dat_control)[, c("type")],
                      Virus = colData(dat_control)[, c("virus")],
                      ID = colData(dat_control)[, c("mcc.id")]),
       col = list(Type = c(Cell_line="lightgrey", Tumor="black", 
                           fibroblasts = "green", keratinocytes = "darkgreen",
                           waga= "darkblue", mkl1 = "blue"), 
                  Virus = vircol,
                  ID = cols2_control), na_col="white")

#dists <- dist(t(log2(counts(dat_control, normalize = TRUE)+1)))
dists <- dist(t(log2(assays(dat_control)$hk_norm+1)))
mat <- as.matrix(dists) # convert to matrix
rownames(mat) <- colnames(mat) <- gsub("NA ", "", with(colData(dat_control), paste0(mcc.id, " ", 
                                       gsub("_", " ", type))))


ht = Heatmap(mat, name = "Distance\n(log2 HK Norm)", col = colors,
    top_annotation = ha_column_control)
draw(ht)
...
Now including control samples. 

```{r heatmap-dist-vs-withcontrol, fig.width=8.5, fig.height=7, fig.path='plots/', dev=c('png')}
distsVS_control <- dist(t(assay(vsd_control)))
mat <- as.matrix(distsVS_control) 
rownames(mat) <- colnames(mat) <- gsub("NA ", "", with(colData(dat_control), paste0(mcc.id, " ", 
                                       gsub("_", " ", type))))
...
```{r heatmap-goi-l2n-patrick-revised-withcontrollines-hk, fig.width=9, fig.height=8, fig.path='plots/', dev=c("png")}
# indices of genes of interest and extra HLA genes
x2 <- which(rowData(dat_control)$symbol %in% symbol)  
 
# subset normalized counts
#mat <- log2(counts(dat_control, normalize = TRUE)[x2,] + 1)
mat <- log2(assays(dat_control)$hk_norm[x2,] + 1)
rownames(mat) <- rowData(dat_control)$symbol[x2]
colnames(mat) <- gsub("NA ", "", with(colData(dat_control), paste0(mcc.id, " ", gsub("_", " ", type)))) 
...
Now including control lines. 

```{r heatmap-goi-vs-patrick-revised-withcontrollines, fig.width=9, fig.height=8, fig.path='plots/', dev=c("png")}
# subset counts of transformed (and normalized) counts
mat <- assay(vst(dat_control))[x2,]
rownames(mat) <- rowData(dat_control)$symbol[x2]
colnames(mat) <- gsub("NA ", "", with(colData(dat_control), paste0(mcc.id, " ", gsub("_", " ", type)))) 
...
if(file.exists(viralquant_file)){
 ...
 # remove 286 and reorder 350, 290, 336, 358, 282, 301, 367, 2314, 277
 viral <- filter(viral, mcc.id != "286")
 viral$mcc.id <- as.factor(as.character(viral$mcc.id))
 neword <- c("350", "290", "336", "358", "282", 
                "301", "367", "2314", "277")
 x <- match(neword, levels(viral$mcc.id))
 viral$mcc.id <- factor(viral$mcc.id, levels(viral$mcc.id)[x])
 ...
 p <- ggplot(viral, aes(x = mcc.id, y = count, fill = type)) +
   ...
   ggtitle("Reads mapping to viral RNA")
 print(p)
 ggsave(file.path("plots", "viralRNAcounts_reordered.pdf"), width = 5, height = 3.5)
}
```
...
Here we examine the fold changes and significance of differential expression between
viral positive and viral negative for the genes of interest.
...
## Difference between IFN responsive and non-responsive

We'll use DESeq2 to perform standard differential expression between the 
IFN responsive and non-responsive samples. We'll adjust for cell line / tumor
origin and viral status as covariates.

```{r volcanoIFNstatus, fig.path='plots/', dev=c('png')}

# filter for samples that have ifn repsonsive vals
dat <- dat[,!is.na(colData(dat)$ifn)]

colData(dat)$ifn <- as.factor(colData(dat)$ifn)

design(dat) <- ~ W + type  + virus + ifn
dat <- DESeq(dat)
...
data.frame(res) %>%  
  ...
  ggplot(aes(log2FoldChange, pvalue, color = DE.01)) +
    ...
    ylab("-log10(p-value)") + 
    scale_colour_manual(values = c("black", "darkorange"))
ggsave(file.path("plots", "volcano_de01_ifnresponse.pdf"), width = 5, height = 4)

# write to file
write.table(res, "plots/deseq2_ifnresponsive.txt", quote=FALSE, row.names=TRUE,
  sep="\t")
```
...
```{r heatmap-DE-l2n-0.01-ifnresponse, fig.width=8.5, fig.height=7, fig.path='plots/', dev=c('png')}
mat <- log2(counts(dat, normalize=TRUE)[res$padj < 0.01 & !is.na(res$padj) & abs(res$log2FoldChange) > 2, ] + 1)
colnames(mat) <- with(colData(dat), paste0(mcc.id, " ", gsub("_", " ", type))) 
...
ha_column = HeatmapAnnotation(df = data.frame(Type = colData(dat)[, c("type")],
                                              Virus = colData(dat)[, c("virus")],
                                              ID = colData(dat)[, c("mcc.id")],
                                              IFN = colData(dat)[, c("ifn")]),
                              col = list(Type = c(Cell_line="lightgrey", Tumor="black"), 
                                         Virus = c(pos = "#543005", neg = "#DFC27D"),
                                         ID = cols2,
                                         IFN = c(Responsive="red", 
                                                 Non_responsive="blue")))
...
```{r heatmap-DE-vs-0.01-ifnresponse, fig.width=8.5, fig.height=7, fig.path='plots/', dev=c('png')}
mat <- assay(vsd)[res$padj < 0.01 & !is.na(res$padj) & abs(res$log2FoldChange) > 2, ]
colnames(mat) <- with(colData(dat), paste0(mcc.id, " ", gsub("_", " ", type))) 
...
```{r gocat-ifnresponse, message=FALSE}
mart <- useMart("ensembl", "hsapiens_gene_ensembl")
goSets <- getBM(c("ensembl_gene_id", "go_id"), mart = mart,
				filters = "ensembl_gene_id", 
				values = unlist(lapply(strsplit(rownames(res), "\\."), 
...
```{r}
gsea_file <- file.path("results", "gsea-ifnresponse.rds")
goSets <- split(rep(names(goSets), lengths(goSets)), unlist(goSets))
stats <- res$stat
names(stats) <- unlist(lapply(strsplit(rownames(res), "\\."), 
				                       function(x) x[1]))
...
```{r gsea-ifnresponse, fig.width=8, fig.height=9, fig.path='plots/', dev=c('png'), warning = FALSE, message = FALSE}
topPathwaysUp <- goRes[ES > 0][head(order(pval), n=10), pathway]
topPathwaysDown <- goRes[ES < 0][head(order(pval), n=10), pathway]
topPathways <- c(topPathwaysUp, rev(topPathwaysDown))
plotGseaTable(goSets[topPathways], stats, goRes, 
...
One of the top categories that DE genes between IFN responsive and non-responsive are overrepresented is related to MHC Class II.

Since many of these pathways are related and overlapping, we also try collapsing
into independent pathways:

```{r gsea-collapsed-ifnresponse, fig.width=8, fig.height=9, fig.path='plots/', warning = FALSE, message = FALSE, dev=c('png')}
collapsedPathways <- collapsePathways(goRes[order(pval)][padj < 0.05], 
                                      goSets, stats)
mainPathways <- goRes[pathway %in% collapsedPathways$mainPathways][
                         order(-NES), pathway][1:20]
...
Here we examine the fold changes and significance of differential expression between
IFN responsive and non-responsive for the genes of interest.
...
```{r volcanogoiifnresponse, fig.path='plots/', dev=c('png')}
p <- ggplot(data = df, aes(log2FoldChange, pvalue, color = DE.01)) +
    geom_point(alpha = 0.4) +
    ylab("-log10(p-value)") + 
    scale_colour_manual(values = c("black", "darkorange")) +
    ...
    geom_label_repel(data = df %>% filter(!is.na(description)) %>% filter(DE.01), 
               aes(label = symbol), color = "black", size = 3)
p
ggsave(file.path("plots", "volcano_de01_goi_ifnresponse.pdf"), plot = p, width = 7, height = 5)
```
# Session Information
