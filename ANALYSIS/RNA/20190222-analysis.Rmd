...
colDat$virus <- "+"
colDat$virus[colDat$mcc.id %in% viralneg] <- "-"
```
...
dat_p <- dat[, c(which(colData(dat)$virus == "+"))]
vsd_p = vst(dat_p)
...
dat_n <- dat[, c(which(colData(dat)$virus %in% "-"))]
vsd_n = vst(dat_n)
...
```{r}
colData(dat)$virus <- factor(ifelse(colData(dat)$virus == "+", "pos", "neg"))
design(dat) <- ~ type + virus
dat <- DESeq(dat)
