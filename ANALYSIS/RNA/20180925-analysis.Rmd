---
title: "MCC RNAseq - compare tumor and cell line"
...
ggsave(file.path("plots", "rawcounts_boxplot.pdf"), width = 6, height = 4)
...
ggsave(file.path("plots", "normcounts_boxplot.pdf"), width = 6, height = 4)
```
...
cols2 <-  colorRampPalette( rev(brewer.pal(6, "Dark2")) )(6)
...
also exclude the virus negative sample (mcc 350), as well as the sample without
a matched cell line sample (mcc 286), as it also appears to be somewhat of an 
outlier:

```{r heatmap-expr-var-500-vs, fig.width=8.5, fig.height=7, fig.path='plots/', dev=c('png', 'pdf')}
dat_s <- dat[, -c(which(colData(dat)$mcc.id %in% c("286", "350")))]
vsd_s = vst(dat_s)

# Calculate distances using transformed (and normalized) counts
# only for top 500 genes
top <- rank(-rowVars(assay(vsd_s)), ties.method = "first")
mat <- assay(vsd_s)[top <= 500,]
colnames(mat) <- with(colData(dat_s), paste0(mcc.id, " ", gsub("_", " ", type))) 
...
samples that are virus positive and have matching cell line data.

```{r}
pcaData <- plotPCA(vsd_s, intgroup=c("type", "mcc.id"), returnData=TRUE)
percentVar <- round(100 * attr(pcaData, "percentVar"))
ggplot(pcaData, aes(PC1, PC2, color=type, shape=mcc.id)) +
  geom_point(size=3) +
  xlab(paste0("PC1: ",percentVar[1],"% variance")) +
...
for (i in seq_along(coverageTracks)){
  ...
  strand(covlist[[i]]) <- "+"
  sumcov <- sum(score(covlist[[i]]))
  score(covlist[[i]]) <- score(covlist[[i]])/sumcov*1e5
...
for (i in seq_along(covlist)){
  peaks[[i]] <- reduce(covlist[[i]][score(covlist[[i]]) > 100])
  if (i == 1)
    int <- peaks[[i]]
  if (i > 1)
...
```{r}
dat <- dat[,-which(colData(dat)$mcc.id == "286")]
dat <- DESeq(dat)
...
There are `r sum(res$padj < 0.01, na.rm = TRUE)` DE genes at FDR 0.01. Here is a 
...
mat <- log2(counts(dat, normalize=TRUE)[res$padj < 0.01 & !is.na(res$padj), ] + 1)
colnames(mat) <- with(colData(dat), paste0(mcc.id, " ", gsub("_", " ", type))) 
...
mat <- assay(vsd)[res$padj < 0.01 & !is.na(res$padj), ]
colnames(mat) <- with(colData(dat), paste0(mcc.id, " ", gsub("_", " ", type))) 
...
are highly related to the immune response. Angiogenesis also seems relevant.
...
We can't do this yet, since we only have one complete sequenced pair that is 
virus negative (350).
