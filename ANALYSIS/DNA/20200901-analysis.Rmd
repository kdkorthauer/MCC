---
title: "MCC Exome seq - compare tumor and cell line: filtered"
...
In this results set, all WES has been filtered for murine contamination with
the [Xengsort tool](https://gitlab.com/genomeinformatics/xengsort/).
...
mafdir <- "../../PREPROCESS/DNA/mutect2-gatk4.1.2.0-results-xengsort/"
...
# write merged maf
prepareMutSig(maf, fn=file.path(mutsigdir, "merged-xengsort"))

maf <- read.maf(file.path(mutsigdir, "merged-xengsort.mutSig.maf"))
...
if (!file.exists(file.path(mutsigdir, 
  "mutsig_results_xengsort.sig_genes.txt"))){
startdir <- getwd()
setwd(file.path(mutsigdir, "MutSigCV_1.41"))
mutsigdir2 <- ".."
system(paste0("matlab -nodisplay -r 'MutSigCV('", 
  file.path(mutsigdir2, "merged-xengsort.mutSig.maf"),  "', '",
  file.path(mutsigdir2, "exome_full192.coverage.txt"), "', '",
  file.path(mutsigdir2, "gene.covariates.txt"), "', '",
  file.path(mutsigdir2, "mutsig_results_xengsort"), "', '",
  file.path(mutsigdir2, "mutation_type_dictionary_file.txt"), "', '",
  file.path(mutsigdir2, "chr_files_hg19"), "')'")
)
setwd(startdir)
}
...
plotEnrichmentResults(enrich, pVal = 0.2)
```
